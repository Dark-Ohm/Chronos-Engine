...
# H2O Heavy-Hitters Design for Tiered KV Offload (Phase 2)
...
|----------|--------|--------------|--------------------------------|-------------|
| **A. Lightweight scoring pass (prefill only)** | H2O §3.1, SnapKV §3.1 | Run a separate attention pass with `causal=false` that outputs per-token scores. Use a small head-dim projection or just mean over heads. | Extra prefill FLOPs: 8 layers × 4 heads × 256² × n_tokens ≈ 0.5-2 GFLOPs per 1K tokens. One extra kernel launch per layer. Negligible vs full prefill. | **Recommended.** Fits llama.cpp graph: add a `build_attn_inp_h2o_score` node that runs once after prefill. |
