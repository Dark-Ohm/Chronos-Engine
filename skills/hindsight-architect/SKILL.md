...
### Self-Hosted / podman Ops (added 2026-07)

For a self-hosted Hindsight dataplane (vectorize-io podman compose: `db`, `embeddings` TEI, `reranker` TEI, `api`, `cp` control-plane, `nginx`), the working surface is NOT the cloud UI. Concrete facts:

- **API root**: `http://localhost:8080` (nginx → `hindsight-api:8888` internally). Dataplane OpenAPI at `/openapi.json`.
- **Dashboard (control-plane)**: `http://localhost:8081` (Next.js — NOT `/`, NOT `/dashboard`; that 404s). It IS bundled in the podman deploy; the architect's job is just to point the user at `:8081`. Exposes bank selector, Memories (world/experience/observations/mental-models), Recall Analyzer, LLM Requests, Audit Logs.
- **MCP transport**: `http://localhost:8080/mcp/{bank_id}/` — gives `recall`/`retain`/`reflect` tools. This is how Zed/Hermes wire in.
- **No cross-bank visibility** — `Banks do not share data` (per official docs). If two clients (e.g. Zed + an agent) must share one memory, they MUST target the SAME bank_id. Separate banks = hard silos, not isolation-by-permission.

**Auth**: self-hosted dataplane ships OPEN by default — `GET /v1/default/banks` and `POST /mcp/{bank}/` return 200 with no key. The container env shows `HINDSIGHT_API_LLM_API_KEY=not-needed`. To lock down, set `HINDSIGHT_API_TENANT_EXTENSION=...ApiKeyTenantExtension` + `HINDSIGHT_API_TENANT_API_KEY=***` (then all requests need `Authorization: Bearer`). nginx is bound `0.0.0.0:8080/8081` by default → LAN-exposed; for a desktop, bind to `127.0.0.1` if it ever leaves localhost.

**LLM backend is the bottleneck.** Hindsight calls `/v1/chat/completions` on `HINDSIGHT_API_LLM_BASE_URL` (e.g. `http://host.containers.internal:8085/v1`) for `retain_extract_facts`, `consolidation`, `reflect`. Pitfalls observed:
- A custom llama.cpp/beellama server with `--cache-type-v turbo2` caused **CUDA illegal memory access** → crashed the model server, all retains STUCK. Fix: `--cache-type-k/v q4_0`.
- MoE layers on CPU (`--n-cpu-moe 40`) dropped throughput to 5–13 t/s. Moving MoE to GPU (`--n-gpu-layers 99 --n-cpu-moe 35`) → 40–45 t/s. retain_extract went from ~300s to ~30s per chunk.
- At low t/s, retain batches from multiple clients compete for the single LLM slot (`--parallel N` helps but one model server is still one queue). Warn the user if two heavy writers (Zed + agent) share one LLM.
- Chat template MUST be supplied (`--chat-template-file ... --jinja`) or the model reports only `completion` capability and Hindsight's structured calls hang.

**Migration** (not live share): `GET /v1/default/banks/{id}/document-transfer` streams a ZIP (facts, entity names, causal links, chunks — embeddings & db-ids NOT included). Import via `POST` multipart `file=@bank.zip` with `?on_conflict=skip`. Used to merge `hermes`+`zed` banks into one shared `chronos-ecosystem` bank. See `references/self-hosted-chronos.md`.

### Mental Model Lifecycle (critical gap)

Mental models are **NOT auto-created** — the architect must create them explicitly, or the bank stays at "raw facts only." They are the Hindsight equivalent of a curated "summary card" (like Hocno/Plastic Labs Peercards).

- **Create**: `POST /v1/default/banks/{bank_id}/mental-models` with `name` + `source_query` (required) and `trigger: {refresh_after_consolidation: true}` (auto-refresh after each retain). Returns `mental_model_id`.
- **Verify**: `GET /v1/default/banks/{bank_id}/mental-models` → `{"items":[]}` means NONE exist. `GET ...observations` (NOT `?limit=1` — that 405s) lists consolidated observations.
- **Count guidance** (from official docs): one model per *recurring query that should return a consistent answer* — not a fixed N. For a single-user project, 1–3 is typical: e.g. `chronos-project-state` (architecture/decisions/status), `user-references` (communication style/expertise), `chronos-build-issues` (recurring build/runtime problems).
- **First refresh is expensive** (multi-step reflect, minutes at ~40 t/s) — warn the user it queues on creation.
- **Retrieval**: `get_mental_model(bank_id, mental_model_id)` is a fast key-value lookup (not an LLM call) — inject `.content` into system prompt. App must store/derive the model ID (naming convention or DB).

When reviewing an existing setup, ALWAYS check `mental-models` count — "no mental models" is the most common gap and the user will read observations/mental-models as their "summaries."
...
## Self-Hosted Detail Reference

Session-specific podman/llama.cpp/endpoint detail from the Chronos integration lives in
`references/self-hosted-chronos.md` (topology, proven live endpoints `:8080`/`:8081`/`:8085`, working llama.cpp flags, the 3 mental_model_ids created, Hermes/Zed wiring, and 6 proven pitfalls). If the file is missing, the essentials are inline above in "Self-Hosted / podman Ops" and "Mental Model Lifecycle".
