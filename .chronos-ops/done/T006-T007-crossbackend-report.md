...
Первоначальная атрибуция («turbo-KV не переживает seq_rm») ОПРОВЕРГНУТА
контрольным прогоном: та же схема на `-ctk f16 -ctv f16` даёт ровно тот же
`unable to reuse common prefix` и полный пересчёт. Значит это НЕ turbo-гэп и
НЕ родня `iq4_nl`-whitelist, а поведение памяти этой модели (Qwythos-9B-MTP)
на частичном совпадении промпта. Корневая причина не названа — заведено
отдельным тикетом, без догадок.

Контроль: `-ctk f16 -ctv f16`, `--prompt-cache-all` на записи, тот же промпт
на чтении -> `session file has exact match for prompt!` +
`unable to reuse common prefix` — идентично turbo-случаю.
