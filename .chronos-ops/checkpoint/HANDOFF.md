...
**Обновлено: 2026-08-16 вечер — origin синхронен на `2c99c6bd4`,
дерево чистое. T003c готов к раздаче завтра, сегодня не стартовать.**

- Продакшн-билд (`build/`) на `e9e4abc6e`: нет T010 и нет
  `d9058908e`. Перед любым live T003c — пересборка с HEAD.
- [[T003c]] постановка дописана (5 пунктов + гибрид + пересборка).
  Раздавать по слову «раздавай», не раньше.
