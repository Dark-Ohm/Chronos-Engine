...
**Обновлено: 2026-08-16 (раздача T003b-pre) — дерево чистое, ветка
`+3` к origin, не пушена. HEAD `adde0f2fe`.**

- [[T003a]] ПРИНЯТ: код `f9f6f0868`, канон+перенос `adde0f2fe`, оба
  файла в `done/`.
- [[T003b-pre]] РАЗДАН (тикет в `active/`). Два вердикта обязательны:
  `tail_groups`-лаг + wraparound на одном `llama_decode` с чанком
  больше кольца (`enqueue_cold_offloads` `:1069`, wraparound
  `:1063-1068`). Живой замер; чтение кода не считается. Ctor не
  трогать. T003c закрыт до вердикта wraparound.
- Свободны параллельно: T004 (P1), T005, T008, T009, T011, T012.
  T003b сам не гейтится T003a; T003c гейтится только pre/wraparound.

---

**Обновлено: 2026-08-16 #8 (чекпоинт) — origin синхронен на `bcfd9c30d`
(#7). В дереве незакоммиченный T003a (11 файлов, +117/-3), НЕ принят.
Не путать с чистым деревом #7. Срез устарел: T003a с тех пор принят.**

## Чекпоинт 2026-08-16 #8 — снимок: T003a на диске, не в git

**HEAD/origin:** `bcfd9c30d` (`repo : чекпоинт #7`). Ahead/behind нет.

**Грязное дерево (T003a, не коммитить как принятое без приёмки):**
`common/arg.cpp`, `common/common.h/.cpp`, `include/llama.h`,
`src/llama-cparams.h`, `src/llama-context.cpp`, `src/llama-model.cpp`,
`src/llama-memory-hybrid.h/.cpp`, `src/llama-kv-cache-kvarn.h/.cpp`.

Что уже лежит в диффе (сверено, не принято):
- Флаг `--kv-h2o-groups` в группах, не токенах; help явно отличает от
  `--kv-hot-size`; слот `cparams.kv_h2o_groups` рядом с `kv_hot_size`,
  не внутри `llama_kvarn_params`.
- Поля per-layer: `h2o_group_flags` packed bits, `h2o_group_scores`
  `[n_head_kv][max_groups]` на слой (слой = `layers[]`). Аллокация
  только при `h2o_enabled`.
- Потолок `n_groups_per_stream / 4`, при клампе `LLAMA_LOG_WARN`.
- Без флага `h2o_groups=0`, буферы пустые.

Приёмка T003a ещё не делалась: критерий «без флага — 0 поведения»
не прогнан живьём в этой сессии.

**Раздача (не изменилась):**
- T003a — в работе (этот дифф).
- T003b-pre — свободен второй руке; два вердикта (лаг + wraparound).
- T003c — после T003a принят + оба вердикта pre.
- Свободны: T004 P1, T005, T008, T009, T011, T012.

**Урок T010 (не терять):** гипотеза тикета опровергнута исполнителем
(кольцо согласовано), причина — гибридный `set_input` не заполнял
`mat_idxs`. Артефакты — в git (`done/t010-live/`), не в `/tmp`.

---

**Обновлено: 2026-08-16 #7 (чекпоинт) — T010 закрыт живьём и запушен,
канон Phase 2 переписан по коду, T003a В РАБОТЕ. Ветка `chronos-main`
синхронна с origin на `4a2c82825`. Дерево чистое.**

## Чекпоинт 2026-08-16 #7 — H2O разблокирован, T003a в работе

**Что произошло после чекпоинта #6:**

1. **[[T010]] — ПРИНЯТ, фикс живой** (`ab222df03`, в done/).
   Гипотеза тикета (рассинхрон index-space store/view) ОПРОВЕРГНУТА
   симуляцией: обе стороны на абсолютных позициях, кольцо консистентно.
   Настоящая причина: Qwythos — гибридная модель (SSM+attention), идёт
   через `build_inp_mem_hybrid()` (`qwen35.cpp:152`), а `mat_idxs`
   заполнялся только в `llm_graph_input_attn_kv::set_input`. Тензор
   оставался НЕинициализированным буфером -> FA-кернелы декодировали
   мусорные позиции. Крашей нет, игла теряется.
   Фикс — 34 строки в `src/llama-graph.cpp`: зеркало паттерна в
   `llm_graph_input_mem_hybrid::set_input`/`can_reuse` + iswa-вариант,
   под гардом `->buffer` (инертен для non-SWA kvarn и не-kvarn).
   Живой A/B: hot 4096 до фикса — иглы нет («lazy lazy» ×234),
   после — `BLUE-ORBIT-7749`, 3/3 прогона, игла в начале и в конце;
   промпт 5602 (кольцо оборачивается) — найдена; без флага регресса нет.
   Артефакты: `.chronos-ops/done/t010-live/` (26 файлов), `t010-sim.py`,
   `t010-gen-niah.py`.

2. **[[T003d]] — РЕШЁН, канон Phase 2 переписан** (`e09b44a55`).
   Три решения, все выведены из кода, а не из SPEC:
   - **Метрика: «H2O с горизонтом стейджа».** Каждый ubatch prefill
     добавляет массу ко ВСЕМ группам, ещё живущим в F16-стейдже; на
     уходе скор замораживается. «Один кадр на флаше» = внимание
     последнего ubatch, НЕ H2O, запрещено. Горизонт = `stage_groups`
     (~1K токенов), а не весь суффикс Zhang. SnapKV/observation window
     отклонены: к концу prefill префикса уже нет (F16 = хвост,
     `host_cold` не читается обратно, promotion нет). Причинность
     структурная -> `causal=false` из Phase 2 удалён.
   - **Удержание: пин в СЖАТОМ GPU-кольце records, не в F16.** Цена
     признана: H2O-группы живут на точности записи, не F16. Пин стоит
     не VRAM, а ОКНА — потолок долей `n_groups_per_stream`.
   - **Отбор per-layer.** `flags[max_groups][n_layers]`,
     `scores[n_kv_heads][max_groups][n_layers]`; GQA — сумма Q-голов
     внутри KV-головы; скоры -> сумма по головам -> top-K внутри слоя.
     Межслойная редукция отклонена (убивает сам сигнал H2O).
   - Следствие: **T003b выведен из предусловий T003c**, prefetch —
     боковая ветка.
   - `--kv-h2o-prefill-only` удалён из спеки (decode-режима нет).
     Реальный флаг Phase 1 — `--kv-hot-size` в ТОКЕНАХ (`arg.cpp:2288`),
     никакого `--kv-hot-groups` не существует.

3. **T003a/b/c приведены к канону** (`515208543`, `4a2c82825`).

**Текущая раздача:**
- **T003a — В РАБОТЕ** (поля + `--kv-h2o-groups`, без кернела/графа).
- **T003b-pre** — гейт снят, можно давать второй рукой. В scope добавлен
  wraparound (`kvarn.cpp:1030-1035`): «avoids this in practice» — это
  предположение, требуется живой репро. ДВА вердикта обязательны (лаг +
  wraparound), один без другого не принимается.
- **T003c** — ждёт T003a + оба вердикта pre. Для него уже записано:
  пространства store/view совпали, live-set пина делается добавлением
  ячеек в ТОТ ЖЕ `mat_idxs`, второе индексное пространство выдумывать
  запрещено.
- Свободны и не пересекаются: **T004** (root cause краша fused, P1),
  **T005**, **T008**, **T009**, **T011**, **T012**.

**Хвост, не тикет:** у T003b в зависимостях вычеркнуто «T003a принят»
(сделано). `llm_graph_input_mem_hybrid_k` не получил зеркало фикса T010 —
его зовёт только `kimi-linear.cpp:246` под `is_mla()`, на Qwythos не едет.
