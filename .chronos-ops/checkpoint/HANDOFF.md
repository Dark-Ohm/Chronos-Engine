...
**Обновлено: 2026-08-16 #9 (чекпоинт) — origin на `2c99c6bd4`,
локально +1 (`5180d59f4`, постановка T003c). Дерево чистое до этого
коммита. T003c не роздан.**

## Чекпоинт 2026-08-16 #9 — H2O плюмбинг и дренаж приняты, T003c ждёт «раздавай»

**HEAD:** `5180d59f4`. **origin/chronos-main:** `2c99c6bd4`. Ahead 1.

**Принято с #8:**
- [[T003a]] `f9f6f0868` + канон `adde0f2fe`. `--kv-h2o-groups`, поля
  per-layer, CLI default 0, cap 1/4 канон. Вне списка тикета приняты
  `iswa` + `memory-hybrid` (боевой путь Qwythos).
- [[T003b-pre]] `d9058908e` + приёмка `75b149e6e`. Лаг подтверждён
  (флаш на группу раньше enqueue). Wraparound реален: «avoids in
  practice» ложь, дефолт сервера `-b 2048`, 48/128 уникальных хэшей;
  после per-ubatch дренажа (`update_lctx` в `init_batch` /
  `init_kv_batch`) — 128/128. Инвариант: `visible > tail + 2`
  (`hot >= 512` при SWA-хвосте 2). `hot <= 384` теряет копии даже с
  фиксом → [[T013]] (не роздан).
- Зонд хэшей: оригинал в git не попал; реконструкция принята
  (`done/t003b-pre-live/t003b-hash-probe.patch`, `CHRONOS_T003B_HASH=1`).

**Правила (не терять):**
- Один тикет в полёте. После каждого захода всплывает то, чего не было
  в постановке.
- Гибридный путь систематически не получает того, что вешают на
  обычный. Обычный: `llm_graph_input_attn_kv::set_input` + `init_batch`.
  Гибрид (Qwythos): `llm_graph_input_mem_hybrid[_iswa]::set_input` +
  `init_kv_batch`. T010 — мусорные `mat_idxs` без краша. T003b-pre —
  порча cold-копий на дефолтном `-b 2048`. Любое новое состояние в
  `set_input` / memory-контексте — живая проверка гибрида в постановке,
  не на приёмке.

**Раздача:**
- [[T003c]] готов, не роздан. Жёстко в тикете: пересборка с HEAD
  (прод `build/` на `e9e4abc6e` — нет T010 и нет дренажа); нет
  `ggml_kvarn_h2o_score` / `build_h2o_scores()`; «H2O с горизонтом
  стейджа»; read-путь (`mat_idxs`/`n_kv`) в том же срезе; 
  `visible_eff = visible - h2o_groups`, гейт `visible_eff > tail + 2`,
  иначе WARN; NIAH после каждого куска; гибрид в постановке.
  Стартовать только по слову «раздавай».
- Лежат: T004 (P1), T013, T005, T008, T009, T011, T012, T003b.
  Параллель не раздавать.

**Урок T010:** гипотеза тикета может врать; принимать по коду и
артефактам в git (`done/t010-live/`, `done/t003b-pre-live/`), не по
скриншоту и не из `/tmp`.

---

**Обновлено: 2026-08-16 (T003b-pre принят) — дерево чистое, ветка
`+6` к origin, не пушена. HEAD `75b149e6e`. Срез устарел: запушено
до `2c99c6bd4`.**

- Правило раздачи: **один тикет в полёте.** После каждого захода
  всплывает то, чего не было в постановке (T010 — гибридный
  `mat_idxs`, T003a — `iswa`+`hybrid` вне списка, T003b-pre — дренаж
  вообще не вызывался из decode-цикла).
- **ПРИНЦИП (не два багфикса, а класс):** *гибридный путь систематически
  не получает того, что вешают на обычный.* Обычный attention идёт через
  `llm_graph_input_attn_kv::set_input` и `init_batch`; гибрид (Qwythos,
  SSM+attention) — через `llm_graph_input_mem_hybrid[_iswa]::set_input`
  и `init_kv_batch`. Дважды за двое суток одно и то же: T010 —
  `mat_idxs` заполнялся только на обычном пути (кернель читал мусор,
  игла терялась без краша); T003b-pre — `update_lctx` передавался
  только на обычном (per-ubatch дренаж cold-копий не работал, копии
  портились в дефолтной конфигурации сервера). **Правило для всех
  будущих тикетов:** любое новое состояние, которое ставится в
  `set_input` или прокидывается в memory-контекст, ОБЯЗАНО быть
  проверено на гибридном пути живьём, и это пишется в постановку, а не
  вспоминается на приёмке.
- [[T003a]] ПРИНЯТ: код `f9f6f0868`, канон+перенос `adde0f2fe`.
- [[T003b-pre]] ПРИНЯТ: фикс `d9058908e`, приёмка `75b149e6e`.
  Вердикт (а) лаг — подтверждён (флаш на группу раньше enqueue).
  Вердикт (б) wraparound — «avoids in practice» опровергнуто живьём:
  48 уникальных хэшей на 128 копий при `-b 2048`; после перевода
  дренажа на per-ubatch — 128/128. Инвариант: `visible > tail + 2`
  (для SWA-хвоста 2 — `hot >= 512`); `hot <= 384` теряет копии даже с
  фиксом → [[T013]].
- [[T003c]] РАЗБЛОКИРОВАН, но НЕ роздан (один в полёте; сам по себе
  волна). При пине помнить: запиненные слоты выводятся из циркуляции,
  то есть `visible_eff = visible - h2o_groups` в том же инварианте.
- Зонд хэшей копий (методика T003b-pre) — оригинал в git не попал,
  реконструкция: `.chronos-ops/done/t003b-pre-live/t003b-hash-probe.patch`
  (`CHRONOS_T003B_HASH=1`, `git apply --check` проходит).
- Свободны: [[T003c]], [[T004]] (P1), [[T013]], T005, T008, T009, T011,
  T012.
