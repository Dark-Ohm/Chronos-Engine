...
- Правило раздачи: **один тикет в полёте.** После каждого захода
  ...
  `mat_idxs`, T003a — `iswa`+`hybrid` вне списка, строки ctor съехали).
  Параллель T004/T005/T008/T009/T011/T012 не раздавать, пока T003b-pre
  не принят.
- [[T003a]] ПРИНЯТ: код `f9f6f0868`, канон+перенос `adde0f2fe`, оба
  файла в `done/`.
- [[T003b-pre]] РАЗДАН — единственный активный. Два вердикта
  обязательны: `tail_groups`-лаг + wraparound на одном `llama_decode`
  с чанком больше кольца (`enqueue_cold_offloads` `:1069`, wraparound
  `:1063-1068`). Живой замер; чтение кода не считается. Ctor не
  трогать.
- [[T003c]] закрыт до вердикта wraparound. Остальное лежит.
