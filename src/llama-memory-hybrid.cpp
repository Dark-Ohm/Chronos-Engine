...
llama_memory_hybrid::llama_memory_hybrid(
    ...
    const layer_filter_cb & filter = filter_attn == nullptr ?
        layer_filter_cb([&](int32_t il) { return !hparams.is_recr(il); })
        : filter_attn;

    if (kvarn.type != LLAMA_KVARN_TYPE_DISABLED) {
        mem_attn.reset(new llama_kv_cache_kvarn(
            model,
            model.hparams,
            kvarn,
            offload,
        ...
        mem_attn.reset(new llama_kv_cache(
            model,
            model.hparams,
            type_k,
            type_v,
...
llama_memory_context_ptr llama_memory_hybrid::init_batch(llama_batch_allocr & balloc, uint32_t n_ubatch, bool embd_all) {
    do {
        ...
        // prepare the attention cache via init_kv_batch (works for both kvarn and non-kvarn)
